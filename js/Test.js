...
firebase.auth().createUserWithEmailAndPassword(email.value, pword.value).then(auth => {
    firebase.storage().ref('vets/' + auth.user.uid + '/profile.jpg').put(file).then(function(){
        console.log('successfully uploded')
        
        firebase.storage().ref('vets/' + auth.user.uid + '/profile.jpg').getDownloadURL()
            .then((url) => {
                db.collection('vet').doc(auth.user.uid).set({
